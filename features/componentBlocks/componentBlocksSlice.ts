...
export const fetchComponentBlocks = createAsyncThunk(
  ...
  async (params: { query?: string; page?: number; perPage?: number; userOnly?: boolean } = {}) => {
    ...
    const res = await fetch(`/api/components${qs ? `?${qs}` : ''}`, { credentials: 'include' })
    if (!res.ok) {
      const text = await res.text()
      throw new Error(text || 'Failed to fetch component blocks')
    }
    const data = (await res.json()) as { componentBlocks: ComponentBlockListItem[]; currentPage: number; total: number }
    return data
  }
)
...
export const { setQuery, setPage, setPerPage, resetList, resetCreate } = componentBlocksSlice.actions
